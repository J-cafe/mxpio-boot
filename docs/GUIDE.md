...
```
应用级覆盖 (mxpio-boot-example/mxpio.properties)    ← 最高优先级
           ↓
...
`mxpio-boot-example` 中的 `mxpio.properties` 可以覆盖任何模块的默认配置。例如，
security 模块默认开启了密码过期和验证码，但 webapp 将其关闭：

```properties
# examples/mxpio-boot-example/src/main/resources/mxpio/mxpio.properties
