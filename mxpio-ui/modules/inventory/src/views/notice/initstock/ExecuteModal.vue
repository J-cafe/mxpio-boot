...
<script lang="ts" setup>
  ...
  import { executeColumns } from './initstock.data';
  import { initstockExecuteApi, isLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'InitstockExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await isLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await initstockExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
