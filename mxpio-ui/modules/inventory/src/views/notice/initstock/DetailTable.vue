...
<script lang="ts" setup>
  ...
  import { isLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';

  defineOptions({ name: 'InitstockDetailTable' });
  ...
  async function setData(data: Recordable) {
    ...
    const res = await isLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const initStockNoticeLines = setDataCrud(data, isUpdate.value);
      return {
        initStockNoticeLines,
      };
    } catch (err) {
      console.error(err);
    }
