...
<script lang="ts" setup>
  ...
  import { executeColumns } from './transferOut.data';
  import { tnExecuteApi, tnLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'TransferOutExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await tnLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await tnExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
