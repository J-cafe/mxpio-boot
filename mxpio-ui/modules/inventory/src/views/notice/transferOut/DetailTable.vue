...
<script lang="ts" setup>
  ...
  import { tnLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'TransferOutNoticeDetailTable',
      keepSource: true,
      minHeight: '200px',
  ...
  async function setData(data: Recordable) {
    ...
    const res = await tnLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const toNoticeLines = setDataCrud(data, isUpdate.value);
      return {
        toNoticeLines,
      };
    } catch (err) {
