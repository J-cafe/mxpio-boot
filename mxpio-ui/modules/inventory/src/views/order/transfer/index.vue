<template>
  <div class="m-3">
    ...
    <TransferOrderModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema } from './transferOrder.data';
  ...
  const componentName = 'TransferOrderList';
  ...
  const {
    ...
    searchFormSchema,
    pageApi: toPageApi,
    deleteApi: deleteToApi,
    configApi: toConfigApi,
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await tolineListApi(row.bizNo);
    row.childList = res;
    return res;
  }
