<template>
  <div class="m-3">
    ...
    <AdjustOutOrderModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema } from './adjustOutOrder.data';
  import AdjustOutOrderModal from './AdjustOutOrderModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'AdjustOutOrderList';
  ...
  async function handleExecute(row: Recordable) {
    try {
      const res = await woIsJitApI(row.bizNo);
      if (res) {
