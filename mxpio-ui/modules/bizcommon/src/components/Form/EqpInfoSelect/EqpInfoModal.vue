...
<script lang="ts" setup>
  ...
  import { eqpCategoryTreeApi, eqpInfoPageApi } from '../../../api';
  ...
  defineOptions({ name: 'EqpInfoModal' });
  ...
  async function loadData({ page, form, sorts }: VxeTableQueryParams) {
    const params = getVxeTableQueryParams({
      ...
      form,
      sorts,
      filters: Object.assign({}, props.filters, { 'eqpTypeCode@EQ': eqpTypeCode.value }),
    });
    const res = await eqpInfoPageApi(params);
    return res;
  }
