...
<script lang="ts" setup>
  ...
  import { executeColumns } from './purchaseReturn.data';
  import { purchaseReturnExecute, rjLineList, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'PurchaseReturnExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await rjLineList(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await purchaseReturnExecute(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
