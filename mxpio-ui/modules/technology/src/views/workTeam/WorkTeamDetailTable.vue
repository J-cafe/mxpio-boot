...
<script lang="ts" setup>
  ...
  import { getWorkteamMember } from '@mxpio/bizcommon';
  import { teamUserColumns } from './workTeam.data';
  import { setDataCrud } from '@mxpio/utils/src/common';

  defineOptions({ name: 'WorkTeamDetailTable' });
  ...
  async function setData(data: Recordable) {
    ...
    const res = await getWorkteamMember(record.workTeamCode);
  ...
  function getData() {
    try {
      ...
      const memberList = setDataCrud(data, isUpdate.value);
      return {
        memberList,
      };
    } catch (err) {
