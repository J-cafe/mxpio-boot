...
<script lang="ts" setup>
  ...
  import { qoPageApi, qoRejectApi, qoLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './purchaseQuality.data';
  import PurchaseQualityModal from './PurchaseQualityModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'PurchaseQualityList';
  const authConfig = {
  ...
  async function handleReject(row: Recordable) {
    try {
      await qoRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await qoLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
