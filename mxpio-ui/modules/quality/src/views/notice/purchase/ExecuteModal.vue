...
<script lang="ts" setup>
  ...
  import { executeColumns } from './purchaseQuality.data';
  import { qoExecuteApi, qoLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await qoLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await qoExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
