...
<script lang="ts" setup>
  ...
  import { iqoLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  async function setData(data: Recordable) {
    ...
    const res = await iqoLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const inQualityOrders = setDataCrud(data, isUpdate.value);
      return {
        inQualityOrders,
      };
    } catch (err) {
