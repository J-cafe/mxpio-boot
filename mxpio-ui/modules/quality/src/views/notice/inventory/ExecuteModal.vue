...
<script lang="ts" setup>
  ...
  import { executeColumns } from './inventoryQuality.data';
  import { iqoExecuteApi, iqoLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'InventoryQualityExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'InventoryQualityExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await iqoLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await iqoExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
