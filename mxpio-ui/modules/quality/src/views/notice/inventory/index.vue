<template>
  <div class="m-3">
    ...
    <InventoryQualityModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { iqoPageApi, iqoRejectApi, iqoLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './inventoryQuality.data';
  import InventoryQualityModal from './InventoryQualityModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'InventoryQualityList';
  const authConfig = {
  ...
  async function handleReject(row: Recordable) {
    try {
      await iqoRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await iqoLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
