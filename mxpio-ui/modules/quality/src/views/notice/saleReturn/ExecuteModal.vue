...
<script lang="ts" setup>
  ...
  import { executeColumns } from './saleReturnQuality.data';
  import { soqoExecuteApi, soqoLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'SoqoQualityExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await soqoLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await soqoExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
