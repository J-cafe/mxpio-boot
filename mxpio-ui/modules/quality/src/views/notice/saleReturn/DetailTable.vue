...
<script lang="ts" setup>
  ...
  import { soqoLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  async function setData(data: Recordable) {
    ...
    const res = await soqoLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const soInQualityOrderLines = setDataCrud(data, isUpdate.value);
      return {
        soInQualityOrderLines,
      };
    } catch (err) {
      console.error(err);
    }
