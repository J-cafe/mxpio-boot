<template>
  <div class="m-3">
    ...
    <SaleReturnQualityModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { soqoPageApi, soqoRejectApi, soqoLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './saleReturnQuality.data';
  import SaleReturnQualityModal from './SaleReturnQualityModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'SaleReturnQualityList';
  const authConfig = {
  ...
  async function handleReject(row: Recordable) {
    try {
      await soqoRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await soqoLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
