...
<script lang="ts" setup>
  ...
  import { executeColumns } from './outsourceQuality.data';
  import { oiqoExecuteApi, oiqoLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'OutsourceQualityExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await oiqoLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await oiqoExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
