<template>
  <div class="m-3">
    ...
    <OutsourceQualityModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { oiqoPageApi, oiqoRejectApi, oiqoLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './outsourceQuality.data';
  import OutsourceQualityModal from './OutsourceQualityModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'OutsourceQualityList';
  const authConfig = {
  ...
  async function handleReject(row: Recordable) {
    try {
      await oiqoRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await oiqoLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
