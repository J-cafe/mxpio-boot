...
<script lang="ts" setup>
  ...
  import { oiqoLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  async function setData(data: Recordable) {
    ...
    const res = await oiqoLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const ooInQualityOrderLines = setDataCrud(data, isUpdate.value);
      return {
        ooInQualityOrderLines,
      };
    } catch (err) {
      console.error(err);
    }
