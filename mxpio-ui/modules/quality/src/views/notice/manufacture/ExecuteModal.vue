...
<script lang="ts" setup>
  ...
  import { executeColumns } from './manufactureQuality.data';
  import { miqoExecuteApi, miqoLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'PurchaseQualityExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'PurchaseQualityExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await miqoLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await miqoExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
