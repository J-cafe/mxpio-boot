...
<script lang="ts" setup>
  ...
  import { miqoLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  async function setData(data: Recordable) {
    ...
    const res = await miqoLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const moInQualityOrderLines = setDataCrud(data, isUpdate.value);
      return {
        moInQualityOrderLines,
      };
    } catch (err) {
      console.error(err);
    }
