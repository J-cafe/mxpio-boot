<template>
  <div class="m-3">
    ...
    <ManufactureQualityModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { miqoPageApi, miqoRejectApi, miqoLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './manufactureQuality.data';
  import ManufactureQualityModal from './ManufactureQualityModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'ManufactureQualityList';
  const authConfig = {
  ...
  async function handleReject(row: Recordable) {
    try {
      await miqoRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await miqoLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
