<template>
  <div>
    ...
    <WithOutItemGroupModal @register="registerModal" @success="handleSuccess" />
  </div>
</template>
<script lang="ts" setup>
  import { useListCrudHook } from '@mxpio/common';
  import { VxeBasicTable, useModal } from '@mxpio/components';
  import { itemGroupColumns } from './byTemplate.data';
  import { useMessage, usePermission } from '@mxpio/hooks';
  import { qtChainItemGroupsWithinApi, qtChainRemoveApi } from '@mxpio/bizcommon';
  import WithOutItemGroupModal from './WithOutItemGroupModal.vue';
  import { watch } from 'vue';

  const componentName = 'QualityChainsWithInItemGroup';
  ...
  const { tableRef, gridOptions, handleSuccess } = useListCrudHook({
    ...
    pageApi: (params) => {
      return qtChainItemGroupsWithinApi(props.currentRow.code, params);
    },
    vxeGridOptions: {
      rowConfig: {
        keyField: 'groupCode',
    ...
    createConfirm({
      ...
      onOk: () => {
        qtChainRemoveApi(
          ...
          selectedRows.map((item) => item.groupCode).join(','),
          '2',
        ).then(() => {
          handleSuccess();
        });
