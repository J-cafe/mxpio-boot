<template>
  <BasicModal
    v-bind="$attrs"
    destroyOnClose
    title="未关联用户"
    width="900px"
    @register="registerModal"
    @ok="handleSubmit"
  >
    <BasicTable @register="registerTable" />
  </BasicModal>
</template>
<script lang="ts" setup>
  import { BasicModal, useModalInner, BasicTable, useTable } from '@mxpio/components';
  import { roleWithoutDept, addRoleActors } from '@mxpio/api';
  import { deptColumns } from './role.data';
  import { getQueryParams } from '@mxpio/utils/src/criteria';
  import { useMessage } from '@mxpio/hooks/src/web/useMessage';
  import type { Key } from 'ant-design-vue/lib/table/interface';
  import type { RequestCriteriaParams } from '@mxpio/types';

  defineOptions({ name: 'RoleWithoutDeptModal' });

  const emit = defineEmits(['success', 'register']);

  const { createMessage } = useMessage();

  const props = defineProps({
    roleId: { type: String, required: true },
  });

  const [registerTable, { reload, getSelectRowKeys }] = useTable({
    api: loadData,
    rowKey: 'id',
    rowSelection: { type: 'checkbox' },
    columns: deptColumns,
    striped: false,
    showTableSetting: true,
    bordered: true,
    canResize: true,
    fetchSetting: {
      sizeField: 'size',
      listField: 'content',
      totalField: 'totalElements',
    },
  });

  const [registerModal, { setModalProps, closeModal }] = useModalInner(async () => {
    setModalProps({ confirmLoading: false });
    reload();
  });

  async function loadData(queryParams) {
    if (props.roleId) {
      const params: RequestCriteriaParams = getQueryParams(queryParams, {});
      const data = await roleWithoutDept(props.roleId, params);
      return data;
    }
    return [];
  }

  async function handleSubmit() {
    try {
      const ids: Key[] = getSelectRowKeys();
      if (ids.length > 0) {
        await addRoleActors(props.roleId, ids);
        closeModal();
        emit('success');
      } else {
        createMessage.success('请选择关联部门');
      }
    } finally {
      setModalProps({ confirmLoading: false });
    }
  }
</script>
