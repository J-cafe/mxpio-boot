<template>
  <PageWrapper contentBackground title="按钮权限控制" contentClass="p-4">
    ...
    <template v-if="isBackPermissionMode">
      ...
      <a-button v-if="hasPermission('sys:menu:add')" color="success" class="mx-4">
        拥有code ['2000']权限可见
      </a-button>
      ...
      <a-button v-auth="'sys:menu:add'" type="primary" class="mx-4">
        拥有code ['sys:menu:add']权限可见
      </a-button>
...
<script lang="ts" setup>
  ...
  import { Authority, PageWrapper } from '@mxpio/components';
  import { usePermissionStore } from '@mxpio/stores/src/modules/permission';
