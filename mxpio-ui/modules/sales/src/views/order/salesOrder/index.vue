<template>
  <div class="m-3">
    <a-card title="销售订单" bordered :bodyStyle="{ padding: '1px' }">
      <VxeBasicTable
        ref="tableRef"
        v-bind="gridOptions"
      ...
      >
        ...
        <template #expand_content="{ row }">
          <!-- <VxeBasicTable ref="lineTable" v-bind="childGridOptions" :data="row.childList" /> -->
...
<script lang="ts" setup>
  ...
  import { columns } from './salesOrder.data';
  ...
  async function handleSubmit(row: Recordable) {
    try {
      await submitSalesOrder(row.bizNo);
  ...
  async function handleAudit(row: Recordable) {
    try {
      await auditSalesOrder(row.bizNo);
      tableRef.value?.commitProxy('query');
    } catch (error) {
      console.log(error);
    }
  ...
  async function handleAbandon(row: Recordable) {
    try {
      await abandonSalesOrder(row.bizNo);
      tableRef.value?.commitProxy('query');
    } catch (error) {
      console.log(error);
    }
  ...
  async function handleClear(row: Recordable) {
    try {
      await clearSalesOrder(row.bizNo);
      tableRef.value?.commitProxy('query');
    } catch (error) {
      console.log(error);
    }
  ...
  async function handleOpen(row: Recordable) {
    try {
      await openSalesOrder(row.bizNo);
      tableRef.value?.commitProxy('query');
    } catch (error) {
      console.log(error);
    }
  ...
  async function handleClose(row: Recordable) {
    try {
      await closeSalesOrder(row.bizNo);
  ...
  async function handleFinish(row: Recordable) {
    try {
      await finishSalesOrder(row.bizNo);
      createMessage.success('操作成功');
      tableRef.value?.commitProxy('query');
    } catch (error) {
      console.log(error);
