<template>
  <div class="m-3">
    ...
    <SaleDeliveryExecuteModal @register="registerExecuteModal" @success="handleSuccess" />
  </div>
</template>
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema } from './saleDelivery.data';
  import SaleDeliveryModal from './SaleDeliveryModal.vue';
  import SaleDeliveryExecuteModal from './SaleDeliveryExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'SaleDeliveryList';
  const authConfig = {
    add: 'erp:SaleDeliveryList:add',
    edit: 'erp:SaleDeliveryList:edit',
    delete: 'erp:SaleDeliveryList:delete',
    import: 'erp:SaleDeliveryList:import',
    export: 'erp:SaleDeliveryList:export',
    execute: 'erp:SaleDeliveryList:execute',
    reject: 'erp:SaleDeliveryList:reject',
  };
  ...
  const {
    ...
    authConfig,
    searchFormSchema,
    pageApi: saleDeliveryPage,
    deleteApi: deleteSaleDelivery,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await saleDeliveryReject(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await snLineList(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
