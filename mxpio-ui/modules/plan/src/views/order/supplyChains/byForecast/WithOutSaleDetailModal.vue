...
<script lang="ts" setup>
  ...
  import { saleDetailOutColumns } from './byForecast.data';
  import type { VxeGridPropTypes, VxeGridInstance, BasicVxeTableProps } from '@mxpio/components';
  import { solineOfPickage, spofSaveApi } from '@mxpio/bizcommon';
  ...
  defineOptions({ name: 'WithOutSaleDetailModal' });

  const emit = defineEmits(['success', 'register']);

  const filters = ref<Recordable>({});
  const tableRef = ref<VxeGridInstance>();
  const forecastRow = ref<Recordable>({});
  const { createMessage } = useMessage();
  const gridOptions = reactive<BasicVxeTableProps>({
    id: 'WithOutSaleDetail',
    keepSource: true,
    minHeight: 300,
    tableClass: '!px-0 !py-0',
    columns: saleDetailOutColumns,
    proxyConfig: {
      ajax: {
      ...
      executeQuantity: [
        { required: true, message: '请输入冲销数量', trigger: 'change' },
        {
          validator: ({ cellValue, row }) => {
            ...
            if (cellValue > forecastRow.value.restQuantity) {
              return new Error('不能大于未冲销预测数量');
            }
          },
          trigger: 'change',
        },
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    filters.value = {
      ...
      'createTime@GE': data.createTime,
    };
    forecastRow.value = data;
  ...
  async function loadData({ page, form, sorts }: VxeTableQueryParams) {
    ...
    const res = await solineOfPickage(params);
    return res;
  }
  ...
  function classifyIntoFormData(selectionRows: Recordable[]) {
    ...
    selectionRows.forEach((item) => {
      data.push({
        ...
        soBizNo: item.bizNo,
        soBizLineNo: item.lineNo,
        spBizNo: forecastRow.value.bizNo,
        spBizLineNo: forecastRow.value.lineNo,
