...
<script lang="ts" setup>
  ...
  import { deleteOomnnApi, oomnPageApi, oomnRejectApi, oomnLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './in.data';
  import PickingModal from './InModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'OutsourceOrderInList';
  ...
  async function handleReject(row: Recordable) {
    try {
      await oomnRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await oomnLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
