...
<script lang="ts" setup>
  ...
  import { oodpLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';

  defineOptions({ name: 'OutsourceScrapDetailTable' });
  ...
  async function setData(data: Recordable) {
    ...
    const res = await oodpLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const ooDefectiveProductsNoticeLines = setDataCrud(data, isUpdate.value);
      return {
        ooDefectiveProductsNoticeLines,
      };
    } catch (err) {
      console.error(err);
    }
