...
<script lang="ts" setup>
  ...
  import { opExecuteApi, opLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'OutsourceSupplementExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'OutsourceSupplementExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await opLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await opExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
