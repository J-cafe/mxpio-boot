...
<script lang="ts" setup>
  ...
  import { mnExecuteApi, mnLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'ManufactureOrderInExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'ManufactureOrderInExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await mnLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await mnExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
