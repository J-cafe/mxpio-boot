<template>
  <div class="m-3">
    ...
    <ScrapModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import {
    deleteManufactureScrapApi,
    manufactureScrapPageApi,
    manufactureScrapRejectApi,
    dpLineListApi,
  } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './scrap.data';
  ...
  const componentName = 'ManufactureScrapList';
  ...
  const {
    ...
    authConfig,
    searchFormSchema,
    pageApi: manufactureScrapPageApi,
    deleteApi: deleteManufactureScrapApi,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await manufactureScrapRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await dpLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
