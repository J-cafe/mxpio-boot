...
<script lang="ts" setup>
  ...
  import { supplementColumns, bomFormSchema } from './outsourceExecute.data';
  ...
  const componentName = 'OutsourceSupplementModal';
  ...
  async function handleSubmit() {
    try {
      ...
      await outsourcePickingApi('46', bizNo.value, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
