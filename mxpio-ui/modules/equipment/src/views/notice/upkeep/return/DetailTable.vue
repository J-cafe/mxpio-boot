...
<script lang="ts" setup>
  ...
  defineOptions({ name: 'OtherReturnDetailTable' });
  ...
  async function setData(data: Recordable) {
    ...
    const res = await eqpUpkeepReturnLineApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const noticeLineList = setDataCrud(data, isUpdate.value);
      return {
        noticeLineList,
      };
    } catch (err) {
