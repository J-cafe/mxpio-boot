...
<script lang="ts" setup>
  ...
  import {
    otherPickupExecuteApi,
    otherPickupLineApi,
    useExecuteClassifyData,
  } from '@mxpio/bizcommon';

  defineOptions({ name: 'OtherPickupExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await otherPickupLineApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await otherPickupExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
