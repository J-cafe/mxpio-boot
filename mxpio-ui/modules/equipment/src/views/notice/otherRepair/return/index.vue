...
<script lang="ts" setup>
  ...
  import {
    deleteOtherReturnApi,
    otherReturnPageApi,
    otherReturnRejectApi,
    otherReturnLineApi,
  ...
  const componentName = 'OtherReturnList';
  ...
  const { tableRef, gridOptions, registerModal, handleDetail, handleSuccess } = useListCrudHook({
    ...
    authConfig,
    searchFormSchema,
    pageApi: otherReturnPageApi,
    deleteApi: deleteOtherReturnApi,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await otherReturnRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await otherReturnLineApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
