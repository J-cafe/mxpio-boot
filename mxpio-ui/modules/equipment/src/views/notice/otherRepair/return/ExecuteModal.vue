...
<script lang="ts" setup>
  ...
  import {
    otherReturnExecuteApi,
    otherReturnLineApi,
    useExecuteClassifyData,
  } from '@mxpio/bizcommon';

  defineOptions({ name: 'OtherReturnExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await otherReturnLineApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await otherReturnExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
