...
<script lang="ts" setup>
  ...
  import {
    upkeepOutsourceInquiryPageApi,
    deleteUpkeepOutsourceInquiryApi,
    upkeepOutsourceInquiryAdoptApi,
  ...
  const {
    ...
    pageApi: upkeepOutsourceInquiryPageApi,
    deleteApi: deleteUpkeepOutsourceInquiryApi,
    vxeGridOptions: {
  ...
  async function handleConfirm(row: Recordable) {
    try {
      await upkeepOutsourceInquiryAdoptApi(row);
      handleSuccess();
      emit('ok');
    } catch (error) {
