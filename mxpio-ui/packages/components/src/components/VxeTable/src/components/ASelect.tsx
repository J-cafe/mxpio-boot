...
function renderOptions(options: any[], optionProps: VxeGlobalRendererHandles.RenderOptionProps) {
  ...
  return XEUtils.map(options, (item, oIndex) => {
    return h(
      ASelectOption as ComponentOptions,
      {
        key: oIndex,
...
function createEditRender() {
  return function (
    ...
    const props = createProps(renderOpts, cellValue, { class: '!w-full' }, params);
          ...
          },
          {
            default: () => {
              return XEUtils.map(optionGroups, (group, gIndex) => {
                return h(
                  ASelectOptGroup as ComponentOptions,
                  {
                    key: gIndex,
                  },
                  {
      ...
      h(
        ...
        optionGroups
          ? column.filters.map((option, oIndex) => {
              ...
              const props = createProps(renderOpts, optionValue, {}, params);
...
function createFloatingFilterRender() {
  return function (
    renderOpts: VxeColumnPropTypes.FilterRender,
    params: VxeGlobalRendererHandles.RenderFilterParams,
  ) {
              ...
              return h(
                ASelect as ComponentOptions,
                {
                  ...
                  ...createEvents(
                    ...
                    () => {
                      // 处理 change 事件相关逻辑
                      $table && $table.updateFilterOptionStatus(option, !!option.data);
