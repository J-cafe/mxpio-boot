...
<script lang="ts" setup>
  ...
  import { buildUUID } from '@mxpio/utils/src/uuid';
  ...
  function beforeUpload(file: File) {
    ...
    if (checkImgType(file)) {
      ...
      fileToBase64(file).then(({ result: thumbUrl }) => {
        fileListRef.value = [
          ...unref(fileListRef),
          {
            thumbUrl,
  ...
  function handleOk() {
    ...
    for (const item of fileListRef.value) {
      ...
      if (status === UploadResultStatus.SUCCESS && response) {
        fileList.push(item?.fileNo || response?.result?.fileNo);
      }
    }
    // 存在一个上传成功的即可保存
    ...
    closeModal();
    emit('change', fileList, { ...fileListRef.value });
    fileListRef.value = [];
  }
