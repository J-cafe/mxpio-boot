<script lang="tsx">
  import { type Recordable, type Nullable } from '@mxpio/types';
  ...
  import { isBoolean, isFunction, isNull } from '@mxpio/utils/src/is';
  import { getSlot } from '@mxpio/utils/src/helper/tsxHelper';
  import {
      ...
      function handleRules(): ValidationRule[] {
        ...
        if (getRequired) {
          if (!rules || rules.length === 0) {
            ...
            //   ? 'blur'
            //   : 'change';
            rules = [{ required: getRequired, validator, trigger: ['change', 'blur'] }];
          } else {
            const requiredIndex: number = rules.findIndex((rule) => Reflect.has(rule, 'required'));

            if (requiredIndex === -1) {
              rules.push({ required: getRequired, validator, trigger: ['change', 'blur'] });
            }
          }
        }
