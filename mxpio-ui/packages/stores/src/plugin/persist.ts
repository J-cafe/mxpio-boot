...
import { getCommonStoragePrefix, Encryption, EncryptionFactory } from '@mxpio/utils';
// import { cacheCipher, SHOULD_ENABLE_STORAGE_ENCRYPTION } from '@mxpio/settings';
...
const persistEncryption: Encryption = EncryptionFactory.createAesEncryption({
  key: encryptionSetting.cacheCipher?.key as string,
  iv: encryptionSetting.cacheCipher?.iv as string,
});
...
export function createPersistedStateOptions(keyPrefix: string): PersistedStateFactoryOptions {
  return {
    ...
    serializer: customSerializer(encryptionSetting.SHOULD_ENABLE_STORAGE_ENCRYPTION as boolean),
  };
}
