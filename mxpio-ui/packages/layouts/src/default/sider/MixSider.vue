<template>
  ...
  >
    ...
    <div :class="`${prefixCls}-menu-list`" ref="sideRef" :style="getMenuStyle">
      ...
      >
        <span class="text"> {{ systemStore.appSystemDesc }}</span>
        <Icon
          :size="16"
...
<script lang="ts" setup>
  ...
  import { useSystemStore, usePermissionStore } from '@mxpio/stores';
  ...
  import { SIDE_BAR_MINI_WIDTH, SIDE_BAR_SHOW_TIT_MINI_WIDTH } from '@mxpio/enums/src/appEnum';
  import vClickOutside from '@mxpio/directives/src/clickOutside';

  import { listenerRouteChange } from '@mxpio/logics/src/mitt/routeChange';
  ...
  const SimpleMenuTag = createAsyncComponent(
    () => import('@mxpio/components/src/components/SimpleMenu/src/SimpleMenuTag.vue'),
  );
  ...
  // const { useSystemStore, usePermissionStore } = useStore(['useSystemStore', 'usePermissionStore']);
