...
import { Persistent, getRawRoute } from '@mxpio/utils';
import { useGo, useRedo } from '../hooks/usePage';
import { projectSetting } from '@mxpio/settings';
import { PageEnum, MULTIPLE_TABS_KEY, RouterEnum } from '@mxpio/enums';
...
export const useMultipleTabStore: StoreDefinition = defineStore({
  id: 'app-multiple-tab',
  ...
  actions: {
    ...
    async addTab(route: RouteLocationNormalized) {
      ...
      if (
        ...
        !name ||
        [RouterEnum.REDIRECT_NAME, RouterEnum.PAGE_NOT_FOUND_NAME].includes(name as RouterEnum)
      ) {
        return;
      }
