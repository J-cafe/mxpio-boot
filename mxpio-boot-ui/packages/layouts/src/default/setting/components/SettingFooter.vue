<template>
  <div :class="prefixCls">
    <a-button type="primary" block @click="handleCopy">
      <CopyOutlined class="mr-2" />
      {{ $t('layout.setting.copyBtn') }}
    </a-button>

    <a-button color="warning" block @click="handleResetSetting" class="my-3">
      <RedoOutlined class="mr-2" />
      {{ $t('common.resetText') }}
    </a-button>

    <a-button color="error" block @click="handleClearAndRedo">
      <RedoOutlined class="mr-2" />
      {{ $t('layout.setting.clearBtn') }}
    </a-button>
  </div>
</template>
<script lang="ts" setup>
  import { unref } from 'vue';

  import { CopyOutlined, RedoOutlined } from '@ant-design/icons-vue';

  // import { useStore } from '@mxpio/bridge';
  import {
    usePermissionStore,
    useMultipleTabStore,
    useUserStore,
    useAppStore,
  } from '@mxpio/stores';
  import { useDesign } from '@mxpio/hooks/src/web/useDesign';
  import { $t } from '@mxpio/locales';
  import { useMessage } from '@mxpio/hooks/src/web/useMessage';
  import { copyText } from '@mxpio/utils/src/copyTextToClipboard';
  import { updateColorWeak } from '@mxpio/logics/src/theme/updateColorWeak';
  import { updateGrayMode } from '@mxpio/logics/src/theme/updateGrayMode';
  import { projectSetting } from '@mxpio/settings/src/projectSetting';
  import { updateSidebarBgColor } from '@mxpio/logics/src/theme/updateBackground';

  defineOptions({ name: 'SettingFooter' });

  // const { usePermissionStore, useMultipleTabStore, useUserStore, useAppStore } = useStore([
  //   'usePermissionStore',
  //   'useMultipleTabStore',
  //   'useUserStore',
  //   'useAppStore',
  // ]);

  const permissionStore = usePermissionStore();
  const { prefixCls } = useDesign('setting-footer');
  const { createSuccessModal, createMessage } = useMessage();
  const tabStore = useMultipleTabStore();
  const userStore = useUserStore();
  const appStore = useAppStore();

  function handleCopy() {
    copyText(JSON.stringify(unref(appStore.getProjectConfig), null, 2), null).then(() => {
      createSuccessModal({
        title: $t('layout.setting.operatingTitle'),
        content: $t('layout.setting.operatingContent'),
      });
    });
  }

  function handleResetSetting() {
    try {
      appStore.setProjectConfig(projectSetting);
      const { colorWeak, grayMode } = projectSetting;
      updateSidebarBgColor();
      updateColorWeak(colorWeak);
      updateGrayMode(grayMode);
      createMessage.success($t('layout.setting.resetSuccess'));
    } catch (error: any) {
      createMessage.error(error);
    }
  }

  function handleClearAndRedo() {
    localStorage.clear();
    appStore.resetAllState();
    permissionStore.resetState();
    tabStore.resetState();
    userStore.resetState();
    location.reload();
  }
</script>
<style lang="less" scoped>
  @prefix-cls: ~'@{namespace}-setting-footer';

  .@{prefix-cls} {
    display: flex;
    flex-direction: column;
    align-items: center;
  }
</style>
