...
import { getViewportOffset } from '@mxpio/utils/src/domUtils';
import { isBoolean } from '@mxpio/utils/src/is';
import { useWindowSizeFn, onMountedOrActivated } from '@mxpio/hooks';
...
export function useTableScroll(
  ...
  watch(
    ...
    async () => {
      ...
      // 等待动画结束后200毫秒
      await promiseTimeout(designSetting?.layoutMultipleHeadePlaceholderTime * 1000 + 200);
      debounceRedoHeight();
    },
    {
      flush: 'post',
  ...
  async function calcTableHeight() {
    ...
    const designSetting = setting.getDesignSetting();
    const layoutFooterHeight = designSetting.footerHeight;
