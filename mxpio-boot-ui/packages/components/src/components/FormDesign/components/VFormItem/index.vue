...
<script lang="ts">
  ...
  // import { componentFDMap } from '../../core/formItemConfig';
  ...
  import formDesignComponents from '../../core/components';
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const formItemProps = computed(() => {
        ...
        let { field, required, rules, labelCol, wrapperCol, componentProps } = unref(props.schema);
        ...
        const newConfig = Object.assign(
          ...
          {
            name:
              !componentProps?.isGlobal && props.bpmnTask?.taskDefinitionKey
                ? `${props.bpmnTask?.taskDefinitionKey}_${field}`
                : field,
            style: { ...style },
            colon,
            required,
        ...
        if (itemProps?.required) {
          newConfig.rules
            ? newConfig.rules?.push({ required: true, message: itemProps?.message })
            : (newConfig.rules = [{ required: true, message: itemProps?.message }]);
        }
        return { ...newConfig };
      }) as Recordable<any>;

      // const componentItem = computed(() => componentFDMap.get(props.schema.component as string));
      const componentItem = computed(() => {
        return formDesignComponents.getComponent(props.schema.component as string);
      });
      ...
      const asyncProps = asyncComputed(async () => {
        ...
        const asyncProps = {};
        options ? (asyncProps['options'] = options) : '';
        treeData ? (asyncProps['treeData'] = treeData) : '';
        return asyncProps;
      });
      ...
      const cmpProps = computed(() => {
        ...
        let { field, componentProps } = props.schema;
      ...
      const handleChange = function (e) {
        ...
        const field =
          !componentProps?.isGlobal && props.bpmnTask?.taskDefinitionKey
            ? `${props.bpmnTask?.taskDefinitionKey}_${props.schema.field}`
            : props.schema.field;
