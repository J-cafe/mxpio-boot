...
<script lang="ts" setup>
  ...
  import { MenuModeEnum, MenuTypeEnum, RouterEnum } from '@mxpio/enums';
  ...
  import { getCurrentAllParentPath } from './hepler/menus';
  import { listenerRouteChange } from '@mxpio/logics/src/mitt/routeChange';
  import { getAllParentPath } from '@mxpio/router-config';
  ...
  listenerRouteChange((route) => {
    if (route.name === RouterEnum.REDIRECT_NAME) return;
  ...
  async function handleMenuChange(route?: RouteLocationNormalizedLoaded) {
    ...
    if (props.isHorizontal && unref(getSplit)) {
      const parentPath = await getCurrentAllParentPath(path);
