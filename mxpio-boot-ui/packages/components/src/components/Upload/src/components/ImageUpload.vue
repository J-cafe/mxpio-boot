...
<script lang="ts" setup>
  ...
  import { isArray, isFunction } from '@mxpio/utils/src/is';
  import { warn } from '@mxpio/utils/src/log';
  import { $t } from '@mxpio/locales';
  import { useUploadType } from '../hooks/useUpload';
  import { uploadContainerProps } from '../props';
  import { isImgTypeByName } from '../helper';
  import { fileToBase64 } from '@mxpio/utils';
  import { UploadResultStatus } from '../../../Upload/src/types/typing';
  import { downloadApi } from '@mxpio/api/src/sys/file';
    ...
    (v) => {
      ...
      if (v) {
        ...
        fileList.value?.forEach((item) => {
          oldPath.push(item.url as string);
        });
        const axiosList: Promise<{ result: string; file: File }>[] = [];
        const fileNos: string[] = [];
        value.forEach((fileNo) => {
          if (!oldPath.includes(fileNo)) {
  ...
  function getValue() {
    const list = (fileList.value || [])
      ...
      .map((item: any) => {
        return item?.fileNo || item?.url || item?.response?.result?.fileNo;
      });
    return props.multiple ? list : list.length > 0 ? list[0] : '';
  }

  async function getImgByFileNo(fileNo) {
