...
<script lang="ts" setup>
  ...
  import { deleteOodpApi, oodpPageApi, oodpRejectApi, oodpLineListApi } from '@mxpio/bizcommon';
  ...
  const componentName = 'OutsourceScrapList';
  ...
  async function handleReject(row: Recordable) {
    try {
      await oodpRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await oodpLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
