...
<script lang="ts" setup>
  ...
  import { executeColumns } from './in.data';
  import { oomnExecuteApi, oomnLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'OutsourceOrderInExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'OutsourceOrderInExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await oomnLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await oomnExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
