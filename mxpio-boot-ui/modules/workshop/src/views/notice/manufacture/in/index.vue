...
<script lang="ts" setup>
  ...
  import { deleteMnApi, mnPageApi, mnRejectApi, mnLineListApi } from '@mxpio/bizcommon';
  ...
  const componentName = 'ManufactureOrderInList';
  ...
  async function handleReject(row: Recordable) {
    try {
      await mnRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await mnLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
