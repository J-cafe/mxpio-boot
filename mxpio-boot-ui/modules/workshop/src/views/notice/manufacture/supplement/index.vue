<template>
  <div class="m-3">
    ...
    <ManufactureSupplementModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { deleteMpApi, mpPageApi, mpRejectApi, mpLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './supplement.data';
  import ManufactureSupplementModal from './SupplementModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'ManufactureSupplementList';
  ...
  const {
    ...
    deleteBefore,
    filters: { 'pickingType@EQ': '42' },
  });

  async function handleReject(row: Recordable) {
    try {
      await mpRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await mpLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
