...
<script lang="ts" setup>
  ...
  import { executeColumns } from './supplement.data';
  import { mpExecuteApi, mpLineListApi, useExecuteClassifyData } from '@mxpio/bizcommon';

  defineOptions({ name: 'ManufactureSupplementExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'ManufactureSupplementExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await mpLineListApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await mpExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
