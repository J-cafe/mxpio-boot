...
<script lang="ts" setup>
  ...
  import { mpLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';

  defineOptions({ name: 'ManufactureReturnDetailTable' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'ManufactureReturnDetailTable',
      keepSource: true,
      minHeight: '200px',
  ...
  async function setData(data: Recordable) {
    ...
    const res = await mpLineListApi(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const moPickingNoticeLines = setDataCrud(data, isUpdate.value);
      return {
        moPickingNoticeLines,
      };
    } catch (err) {
      console.error(err);
    }
