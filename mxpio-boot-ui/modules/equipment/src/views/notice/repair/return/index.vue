...
<script lang="ts" setup>
  ...
  import {
    deleteRepairReturnApi,
    repairReturnPageApi,
    repairReturnRejectApi,
    repairReturnLineApi,
  ...
  const componentName = 'RepairReturnNoticeList';
  ...
  const { tableRef, gridOptions, registerModal, handleDetail, handleSuccess } = useListCrudHook({
    ...
    authConfig,
    searchFormSchema,
    pageApi: repairReturnPageApi,
    deleteApi: deleteRepairReturnApi,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await repairReturnRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await repairReturnLineApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
