...
<script lang="ts" setup>
  ...
  import {
    repairReturnExecuteApi,
    repairReturnLineApi,
    useExecuteClassifyData,
  } from '@mxpio/bizcommon';

  defineOptions({ name: 'RepairReturnExecuteModal' });
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await repairReturnLineApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await repairReturnExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
