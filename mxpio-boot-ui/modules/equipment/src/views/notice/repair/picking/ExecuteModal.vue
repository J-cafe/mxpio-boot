...
<script lang="ts" setup>
  ...
  import {
    repairPickupExecuteApi,
    repairPickupLineApi,
    useExecuteClassifyData,
  } from '@mxpio/bizcommon';

  defineOptions({ name: 'RepairPickupExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'RepairPickupNoticeExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await repairPickupLineApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await repairPickupExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
