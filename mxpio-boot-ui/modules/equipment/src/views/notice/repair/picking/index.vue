...
<script lang="ts" setup>
  ...
  import {
    deleteRepairPickupApi,
    repairPickupPageApi,
    repairPickupRejectApi,
    repairPickupLineApi,
  ...
  const componentName = 'RepairPickupNoticeList';
  ...
  const { tableRef, gridOptions, registerModal, handleDetail, handleSuccess } = useListCrudHook({
    ...
    authConfig,
    searchFormSchema,
    pageApi: repairPickupPageApi,
    deleteApi: deleteRepairPickupApi,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await repairPickupRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await repairPickupLineApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
