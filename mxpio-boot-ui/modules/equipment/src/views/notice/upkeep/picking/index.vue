...
<script lang="ts" setup>
  ...
  import {
    deleteUpkeepPickupApi,
    upkeepPickupPageApi,
    upkeepPickupRejectApi,
    upkeepPickupLineApi,
  ...
  const componentName = 'UpkeepPickupNoticeList';
  ...
  const { tableRef, gridOptions, registerModal, handleDetail, handleSuccess } = useListCrudHook({
    ...
    authConfig,
    searchFormSchema,
    pageApi: upkeepPickupPageApi,
    deleteApi: deleteUpkeepPickupApi,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await upkeepPickupRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await upkeepPickupLineApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
