...
<script lang="ts" setup>
  ...
  import {
    eqpUpkeepReturnExecuteApi,
    eqpUpkeepReturnLineApi,
    useExecuteClassifyData,
  } from '@mxpio/bizcommon';

  defineOptions({ name: 'EqpUpkeepReturnExecuteModal' });
  ...
  const gridOptions = computed<BasicVxeTableProps>(() => {
    return {
      id: 'EqpUpkeepReturnExecuteTable',
      keepSource: true,
      minHeight: '200px',
  ...
  const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
    ...
    notice = data.record;
    const res = await eqpUpkeepReturnLineApi(notice.noticeNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await eqpUpkeepReturnExecuteApi(notice.noticeNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
