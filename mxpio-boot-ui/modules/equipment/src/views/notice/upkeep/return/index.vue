...
<script lang="ts" setup>
  ...
  import {
    deleteEqpUpkeepReturnApi,
    eqpUpkeepReturnPageApi,
    eqpUpkeepReturnRejectApi,
    eqpUpkeepReturnLineApi,
  } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './return.data';
  import PickingModal from './ReturnModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'EqpUpkeepReturnList';
  ...
  const { tableRef, gridOptions, registerModal, handleDetail, handleSuccess } = useListCrudHook({
    ...
    authConfig,
    searchFormSchema,
    pageApi: eqpUpkeepReturnPageApi,
    deleteApi: deleteEqpUpkeepReturnApi,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await eqpUpkeepReturnRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await eqpUpkeepReturnLineApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
