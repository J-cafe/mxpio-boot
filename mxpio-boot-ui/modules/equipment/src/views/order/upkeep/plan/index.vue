<template>
  <div class="m-3 bg-white">
    ...
    <a-tabs class="px-6">
      <a-tab-pane key="1" tab="保养计划目标">
        <TargetTable :planId="currentRow.id" :plan="currentRow" />
      </a-tab-pane>
      <a-tab-pane key="2" tab="保养计划明细">
        <DetailTable :planId="currentRow.id" :plan="currentRow" />
      </a-tab-pane>
    </a-tabs>
    <UpkeepPlanModal width="80%" @register="registerModal" @success="handleSuccess" />
  </div>
</template>
<script lang="ts" setup>
  ...
  import {
    deleteEqpUpkeepPlanApi,
    eqpUpkeepPlanPageApi,
    enableEqpUpkeepPlanApi,
    recoverEqpUpkeepPlanApi,
    pauseEqpUpkeepPlanApi,
    disableEqpUpkeepPlanApi,
    copyEqpUpkeepPlanApi,
  } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './upkeepPlan.data';
  import UpkeepPlanModal from './UpkeepPlanModal.vue';
  import DetailTable from './DetailTable.vue';
  import TargetTable from './TargetTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'EqpUpkeepPlanList';
  ...
  const {
    ...
    columns,
    searchFormSchema,
    pageApi: eqpUpkeepPlanPageApi,
    deleteApi: deleteEqpUpkeepPlanApi,
    vxeGridOptions: {
      height: 450,
      tableClass: 'pb-0',
      rowConfig: {
  ...
  async function handleEnable(row) {
    try {
      await enableEqpUpkeepPlanApi(row.id);
  ...
  async function handleRecover(row) {
    try {
      await recoverEqpUpkeepPlanApi(row.id);
      createMessage.success('操作成功');
      handleSuccess();
    } catch (error) {
      console.log(error);
  ...
  async function handlePause(row) {
    try {
      await pauseEqpUpkeepPlanApi(row.id);
      createMessage.success('操作成功');
      handleSuccess();
    } catch (error) {
      console.log(error);
  ...
  async function handleDisable(row) {
    try {
      await disableEqpUpkeepPlanApi(row.id);
      createMessage.success('操作成功');
      handleSuccess();
    } catch (error) {
      console.log(error);
  ...
  async function handleCopy() {
    try {
      ...
      await copyEqpUpkeepPlanApi(currentRow.value.id);
      createMessage.success('操作成功');
      handleSuccess();
    } catch (error) {
      console.log(error);
