...
<script lang="ts" setup>
  ...
  import {
    upkeepOutsourcePageApi,
    upkeepOutsourceIssuedApi,
    upkeepOutsourceRejectApi,
    upkeepOutsourceInquiryEndApi,
  ...
  const componentName = 'UpkeepOutsourceList';
  ...
  const { tableRef, gridOptions, handleDetail, handleSuccess } = useListCrudHook({
    componentName,
    columns,
    searchFormSchema,
    pageApi: upkeepOutsourcePageApi,
    vxeGridOptions: {
      height: 450,
      rowConfig: {
        keyField: 'bizNo',
  ...
  async function handleIssusd(row) {
    try {
      await upkeepOutsourceIssuedApi(row);
      handleSuccess();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleReject = async (row) => {
    try {
      await upkeepOutsourceRejectApi(row);
      handleSuccess();
    } catch (error) {
      console.log(error);
    }
  ...
  async function handleInquiry() {
    try {
      ...
      await upkeepOutsourceInquiryEndApi(currentRow.value.bizNo);
      handleSuccess();
    } catch (error) {
      console.log(error);
    }
