...
<script lang="ts" setup>
  ...
  import {
    returnOtherRepairApi,
    pickingOtherRepairLineApi,
    useExecuteClassifyData,
  } from '@mxpio/bizcommon';
  import { useMessage } from '@mxpio/hooks';
  import Big from 'big.js';

  defineOptions({ name: 'OtherRepairReturnModal' });
  ...
  async function getLines() {
    const res = await pickingOtherRepairLineApi(bizNo);
  ...
  async function handleSubmit() {
    try {
      ...
      await returnOtherRepairApi(bizNo, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
