...
<script lang="ts" setup>
  ...
  import {
    otherOutsourcePageApi,
    otherOutsourceIssuedApi,
    otherOutsourceRejectApi,
    otherOutsourceInquiryEndApi,
  ...
  const componentName = 'OtherRepairOutsourceList';
  ...
  async function handleIssusd(row) {
    try {
      await otherOutsourceIssuedApi(row);
      handleSuccess();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleReject = async (row) => {
    try {
      await otherOutsourceRejectApi(row);
      handleSuccess();
    } catch (error) {
      console.log(error);
    }
  ...
  async function handleInquiry() {
    try {
      ...
      await otherOutsourceInquiryEndApi(currentRow.value.bizNo);
      handleSuccess();
    } catch (error) {
      console.log(error);
    }
