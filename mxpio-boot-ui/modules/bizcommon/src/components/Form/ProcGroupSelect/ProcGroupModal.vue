...
<script lang="ts" setup>
  ...
  import { procGroupPage } from '../../../api/technology/procGroup';
  ...
  defineOptions({ name: 'ProcGroupSelectModal' });
  ...
  async function loadData({ page, form, sorts }: VxeTableQueryParams) {
    ...
    const res = await procGroupPage(params);
    return res;
  }
  ...
  async function handleSubmit() {
    try {
      if (props.multiple) {
        ...
        const ids = rows?.map((item) => item.procGroupCode);
        emit('success', ids?.join(','), rows);
      } else {
        const row = tableRef.value?.getRadioRecord(true);
        emit('success', row.procGroupCode, row);
      }
      closeModal();
    } finally {
      setModalProps({ confirmLoading: false });
