...
<script lang="ts" setup>
  ...
  import { saleDetailColumns } from './byForecast.data';
  import { useMessage, usePermission } from '@mxpio/hooks';
  import { spofDeleteApi, spofPageApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import WithOutSupplierModal from './WithOutSaleDetailModal.vue';

  const componentName = 'WithInSaleDetail';
  ...
  const auth = {
    add: `erp:SupplyChainsByForecast:add`,
    edit: `erp:SupplyChainsByForecast:edit`,
    delete: `erp:SupplyChainsByForecast:delete`,
  };
  ...
  const { tableRef, gridOptions, checkboxChange } = useListCrudHook({
    ...
    filters: () => {
      return {
        ...
        'spBizNo@EQ': props.currentRow.bizNo,
        'spBizLineNo@EQ': props.currentRow.lineNo,
      };
    },
    module: 'erp',
  });
