...
<script lang="ts" setup>
  ...
  import { odLineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  async function setData(data: Recordable) {
    ...
    const res = await odLineListApi(record.bizNo);
  ...
  function getData() {
    try {
      ...
      const organizationDemandLineList = setDataCrud(data, isUpdate.value);
      return {
        organizationDemandLineList,
      };
    } catch (err) {
      console.error(err);
    }
