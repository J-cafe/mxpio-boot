...
<script lang="ts" setup>
  ...
  import { pickingColumns } from './organizeNeedDetail.data';
  import { odReceiveApi, useExecuteClassifyData } from '@mxpio/bizcommon';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'PickingModal';
  ...
  async function handleSubmit() {
    try {
      ...
      await odReceiveApi(bizNo.value, executeData);
      closeModal();
      emit('success');
    } finally {
      setModalProps({ confirmLoading: false });
