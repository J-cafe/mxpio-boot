...
<script lang="ts" setup>
  ...
  import { odLinesColumns, odSearchFormSchema } from './mainPlan.data';
  import { useListCrudHook } from '@mxpio/common';
  import { mpsOdPageApi } from '@mxpio/bizcommon';
  import { useMessage } from '@mxpio/hooks';
  import type { RequestCriteriaParams } from '@mxpio/types';

  const componentName = 'OrganizeOredrLinesModal';
  ...
  const { tableRef, gridOptions } = useListCrudHook({
    componentName,
    module: 'erp',
    columns: odLinesColumns,
    searchFormSchema: odSearchFormSchema,
    pageApi: async (params?: RequestCriteriaParams) => {
      if (params) {
        params.versionCode = versionCode.value;
      }
      const res = await mpsOdPageApi(params);
      return res;
    },
    vxeGridOptions: {
      toolbarConfig: {
