<template>
  <div class="m-3">
    ...
    <PurchaseReceiveModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { columns, searchFormSchema } from './purchaseReceive.data';
  ...
  const componentName = 'PurchaseReceiveList';
  const authConfig = {
    add: 'erp:PurchaseReceiveList:add',
    edit: 'erp:PurchaseReceiveList:edit',
    delete: 'erp:PurchaseReceiveList:delete',
    import: 'erp:PurchaseReceiveList:import',
    export: 'erp:PurchaseReceiveList:export',
    execute: 'erp:PurchaseReceiveList:execute',
    reject: 'erp:PurchaseReceiveList:reject',
  };
  ...
  const {
    ...
    authConfig,
    searchFormSchema,
    pageApi: purchaseReceivePage,
    deleteApi: deletePurchaseReceive,
    vxeGridOptions: {
      rowConfig: {
        keyField: 'noticeNo',
      },
  ...
  async function handleReject(row: Recordable) {
    try {
      await purchaseReceiveReject(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await roLineList(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
