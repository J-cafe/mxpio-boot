...
<script lang="ts" setup>
  ...
  import { roLineList, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';
  ...
  async function setData(data: Recordable) {
    ...
    const res = await roLineList(record.noticeNo);
  ...
  function getData() {
    try {
      ...
      const receivingOrderLines = setDataCrud(data, isUpdate.value);
      return {
        receivingOrderLines,
      };
    } catch (err) {
