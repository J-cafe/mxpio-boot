...
<script lang="ts" setup>
  ...
  import { tolineListApi, positiveNumberPattern, useMaterialLotSelect } from '@mxpio/bizcommon';

  defineOptions({ name: 'TransferOrderDetailTable' });
  ...
  const { materialSelectConfig, lotSelectConfig, getInsertData } = useMaterialLotSelect({
    ...
    extraLotProps: (row) => ({
      filters: {
        whCode: props.formData.originWhCode,
        itemCode: row.itemCode,
      },
      disabled: !props.formData.originWhCode,
      multiple: false,
    }),
    defaultValues: {
      closeStatus: 'open',
  ...
  const detailColumns: VxeGridPropTypes.Columns = [
    ...
    { title: '计划出库数量', field: 'planOutQuantity' },
    { title: '已出库数量', field: 'actualOutQuantity' },
    { title: '计划入库数量', field: 'planInQuantity' },
    { title: '已入库数量', field: 'actualInQuantity' },
    {
      title: '备注',
      field: 'memo',
      editRender: {
  ...
  async function setData(data: Recordable) {
    ...
    const res = await tolineListApi(record.bizNo);
  ...
  function getData() {
    try {
      ...
      const transferOrderLines = setDataCrud(data, isUpdate.value);
      return {
        transferOrderLines,
      };
    } catch (err) {
