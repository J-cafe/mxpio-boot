<template>
  <div class="m-3">
    ...
    <TransferOutNoticeModal @register="registerModal" @success="handleSuccess" />
...
<script lang="ts" setup>
  ...
  import { deleteTnApi, tnPageApi, tnRejectApi, tnLineListApi } from '@mxpio/bizcommon';
  import { useListCrudHook } from '@mxpio/common';
  import { columns, searchFormSchema } from './transferOut.data';
  import TransferOutNoticeModal from './TransferOutNoticeModal.vue';
  import ExecuteModal from './ExecuteModal.vue';
  import ExpandTable from './ExpandTable.vue';
  import { useMessage } from '@mxpio/hooks';

  const componentName = 'TransferOutNoticeList';
  ...
  async function handleReject(row: Recordable) {
    try {
      await tnRejectApi(row.noticeNo);
  ...
  async function loadContentMethod({ row }: { row: Recordable }) {
    const res = await tnLineListApi(row.noticeNo);
    row.childList = res;
    return res;
  }
</script>
